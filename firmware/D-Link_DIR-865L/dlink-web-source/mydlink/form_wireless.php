...
if($settingsChanged == 1)
{
  ...
  $phy_1  = XNODE_getpathbytarget("", "phyinf", "uid", "BAND24G-1.1", 0);
  $wifi_1 = XNODE_getpathbytarget("/wifi", "entry", "uid", query($phy_1."/wifi"), 0);

  $phy_2  = XNODE_getpathbytarget("", "phyinf", "uid", "BAND5G-1.1", 0);
  $wifi_2 = XNODE_getpathbytarget("/wifi", "entry", "uid", query($phy_2."/wifi"), 0);
	
	foreach("/wifi")
  {
	  if(query($phy_1."/active") == 1 && $new_auth == "WPA" && $new_wpa_type == "PSK")
	  {
	    set($wifi_1."/wps/enable", 0);
	    set($wifi_2."/wps/enable", 0);
	  }  	 
	  else if(query($phy_2."/active") == 1 && $new_auth == "WPA" && $new_wpa_type == "PSK")
	  {
		  set($wifi_2."/wps/enable", 0);
		  set($wifi_1."/wps/enable", 0);
	  }
	  else if(query($phy_1."/active") == 1 && $new_auth == "WEP")
	  {
		  set($wifi_1."/wps/enable", 0);
		  set($wifi_2."/wps/enable", 0);
	  }
	  else if(query($phy_2."/active") == 1 && $new_auth == "WEP")
	  {
		  set($wifi_2."/wps/enable", 0);
		  set($wifi_1."/wps/enable", 0);
	  }
