...
function mdb_get($cmd_name,$WAN1,$WLAN1)
{
	...
	else if($cmd_name == "pppoe_info")
	{
		...
		anchor($path_inet_wan1."/ppp4");
		$pppoe_enable = query($mydlink_path."/ppp4/pppoe/enable");
		if($pppoe_enable == "") $pppoe_enable = 0;
		$username = UrlEncode(query("username"));
		$password = UrlEncode(query("password"));
	...
	else if($cmd_name == "wlan_info")
	{
		...
		$ssid = UrlEncode(query($path_wifi_wlan1."/ssid"));
		$str_auth = query($path_wifi_wlan1."/authtype");
		$str_encrypt = query($path_wifi_wlan1."/encrtype");
		...
		if($str_encrypt == "WEP")
		{
			if(query($path_wifi_wlan1."/nwkey/wep/size") == 64) {$encrypt_type = 0;}
			else 												{$encrypt_type = 1;}			
		}
		...
		if($str_encrypt == "WEP") 			   {$key = UrlEncode(query($path_wifi_wlan1."/nwkey/wep/key:1"));}
		else if(strstr($str_auth,"PSK") != "") {$key = UrlEncode(query($path_wifi_wlan1."/nwkey/psk/key"));}
	...
	foreach($mdb_tmp_path."/entry")
	{
		...
		else if($cmd_name == "pppoe_info")
		{
			...
			while ($i < $cnt)
			{
				...
				if (charcodeat($token,0) == "E") // Enable or disable the PPPoE 
				{
					...
					set($mydlink_path."/ppp4/pppoe/enable",$Value);
				}
		...
		else if($cmd_name == "wlan_info")
		{
			...
			while ($i < $cnt)
			{
				...
				{
					$Value = cut($token, 1, $delimiter2);
					set($path_wifi_wlan1."/ssid", $Value);
				}
				else if (charcodeat($token,0) == "S") // Security type
				{
					...
					if($Value == 0) //None
					{
						set($path_wifi_wlan1."/authtype", "OPEN");
						set($path_wifi_wlan1."/encrtype", "NONE");
					}
					...
					{
						set($path_wifi_wlan1."/authtype", "SHARED");
					}
					else if($Value == 3) //WPA-PSK
					{
						set($path_wifi_wlan1."/authtype", "WPAPSK");
					}
					else if($Value == 4) //WPA2-PSK
					{
						set($path_wifi_wlan1."/authtype", "WPA2PSK");
					}
					else if($Value == 5) //WPA+2PSK
					{
						set($path_wifi_wlan1."/authtype", "WPA+2PSK");
					}
					else //Default
					{
						set($path_wifi_wlan1."/authtype", "OPEN");
				...
				else if (charcodeat($token,0) == "E") //  Encryption type
				{
					...
					{
						$is_wep = 1;
						set($path_wifi_wlan1."/encrtype", "WEP");
						set($path_wifi_wlan1."/nwkey/wep/size",64);
						set($path_wifi_wlan1."/nwkey/wep/ascii",1);
					}
					else if($Value == 1) // WEP, 128bit 
					{
						$is_wep = 1;
						set($path_wifi_wlan1."/encrtype", "WEP");
						set($path_wifi_wlan1."/nwkey/wep/size",128);
						set($path_wifi_wlan1."/nwkey/wep/ascii",1);
					}
					else if($Value == 2) // AES
					{
						set($path_wifi_wlan1."/encrtype", "AES");
					}
					else if($Value == 3) // TKIP
					{
						set($path_wifi_wlan1."/encrtype", "TKIP");
					}
					else if($Value == 4) // TKIP or AES
					{
						set($path_wifi_wlan1."/encrtype", "TKIP+AES");
					}
					else //Default
					{
						set($path_wifi_wlan1."/encrtype", "NONE");
				...
				else if (charcodeat($token,0) == "K") //  Encryption key
				{
					...
					if($is_wep == 1)
					{
						set($path_wifi_wlan1."/nwkey/wep/defkey",1);
						set($path_wifi_wlan1."/nwkey/wep/key:1",$Value);
					}
					else
					{
						set($path_wifi_wlan1."/nwkey/psk/key", $Value);
					}
				}
				$i++;
			}
